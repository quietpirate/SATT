...
def tcpScans(targetRange, viewpoint, initialRTT, tcpMaxRTT, outputName, tcpMaxHostGroup, tcpMinHostGroup, maxRetries, minRate, excludeHosts):
    if viewpoint == 'internal':
        ...
        p = subprocess.Popen("sudo nmap -d -Pn --stats-every 10s --reason -sT --max-hostgroup 2 --min-hostgroup 2 --max-retries 7  --max-scan-delay 25ms -p T:0-65535  --max-rtt-timeout "+ tcpMaxRTT + "ms --initial-rtt-timeout "+ initialRTT + "ms -oA ./tcp/" + outputName + "_tcp " + targetRange, shell=True)
        p.communicate()
...
def udpScans(targetRange, viewpoint, initialRTT, udpMaxRTT, outputName, udpMaxHostGroup, udpMinHostGroup, udpMaxRetries, minRate, excludeHosts):
    if viewpoint == 'internal':
        ...
        p = subprocess.Popen("sudo nmap -d -Pn --reason --stats-every 10s --top-ports 5000 -sUV -n --max-retries " + udpMaxRetries + " --min-hostgroup " + udpMinHostGroup + " --max-hostgroup " + udpMaxHostGroup + " --version-intensity 0 --exclude " + excludeHosts + " --max-rtt-timeout " + udpMaxRTT + "ms --max-scan-delay 25ms --initial-rtt-timeout "+ initialRTT + "ms -oA ./udp/" + outputName + "_udp " + targetRange, shell=True)
        p.communicate()

    else:
        print "Starting external UDP Port scan"
        p = subprocess.Popen("sudo nmap --reason -d -Pn --stats-every 10s --top-ports 3000 -sUV --max-retries "+ udpMaxRetries +" --version-intensity 0 --max-rtt-timeout " + udpMaxRTT + " --initial-rtt-timeout " + initialRTT + " -oA ./udp/" + outputName + "_udp " + targetRange, shell=True)
        p.communicate()

    print "UDP scanning done."
    return True

def printScans(targetRange, viewpoint, avgRTT, tcpMaxRTT, outputName, tcpMaxHostGroup, tcpMinHostGroup, maxRetries, minRate, excludeHosts, udpMaxRTT, udpMaxHostGroup, udpMinHostGroup):
    print colored("Discovery Scans: ", 'green')
    print "sudo nmap --reason -d -sn --stats-every 10s -n -PE --exclude " + excludeHosts + " -oA ./discovery/" + outputName + "_icmp " + targetRange

    print colored("\nAck Scans:", 'green')
    print "sudo nmap --reason -d -sn --stats-every 10s -n -PA7,9,13,21-23,25-26,37,53,79-81,88,106,110-111,113,119,135,139,143-144,179,199,389,427,443-445,465,513-515,543-544,548,554,587,631,646,873,990,993,995,1025-1029,1110,1433,1720,1723,1755,1900,2000-2001,2049,2121,2717,3000,3128,3306,3389,3986,4899,5000,5009,5051,5060,5101,5190,5357,5432,5631,5666,5800,5900,6000-6001,6646,7070,8000,8008-8009,8080-8081,8443,8888,9100,9999-10000,32768,49152-49157 --initial-rtt-timeout 5ms --exclude " + excludeHosts + " -oA ./discovery/" + outputName + "_ack " + targetRange
    ...
    print colored("\nRun TCP Scan: ", 'green')
    print "sudo nmap -d -Pn --stats-every 10s -n --reason -sT --max-hostgroup " + tcpMaxHostGroup + " --min-hostgroup " + tcpMinHostGroup + " --max-retries " + maxRetries + " --min-rate " + minRate + " -p T:0-65535 --max-scan-delay 25ms --exclude " + excludeHosts + " --max-rtt-timeout "+ tcpMaxRTT + "ms --initial-rtt-timeout "+ avgRTT + "ms -oA ./tcp/" + outputName + "_tcp " + targetRange

    print colored("\nRun UDP Scan: ", 'green')
    print "sudo nmap -d -Pn --reason --stats-every 10s --top-ports 5000 -sUV -n --max-retries " + maxRetries + " --min-hostgroup " + udpMinHostGroup + " --max-hostgroup " + udpMaxHostGroup + " --version-intensity 0 --exclude " + excludeHosts + " --max-rtt-timeout " + udpMaxRTT + "ms --max-scan-delay 25ms --initial-rtt-timeout "+ avgRTT + "ms -oA ./udp/" + outputName + "_udp " + targetRange
    return True

def main(targetRange, sourceIP, viewpoint, configFile):
    ...
    if printCommands == True:
        ...
        if excludeHosts == None:
            excludeHosts = ""
        printScans(targetRange, viewpoint, avgRTT, tcpMaxRTT, outputName, tcpMaxHostGroup, tcpMinHostGroup, maxRetries, min_rate, excludeHosts, udpMaxRTT, udpMaxHostGroup, udpMinHostGroup)
    else:
        fullList = host_discovery(targetRange, viewpoint, outputName, excludeHosts)
        if len(fullList) == 0:
            print "No hosts found"
            minRTT, avgRTT, maxRTT = 15
        else:
